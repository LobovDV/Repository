'use strict';
    $(document).ready(function (){
        $('.flag-icon-russian').click(function (){
          window.location.replace('?lang=ru');
        });
        $('.flag-icon-english').click(function (){
            window.location.replace('?lang=en');
        });
        $('.all-tags-link').click(function (){
            $('.tagsHeader').css('display', 'inline-block');
        });


        $('.GenreSection').ready( function () {
            $('.Tags').addClass( function() {
                if ($(this).children('.Tags').length > 0) {
                    $(this).addClass("Tags_embed");
                }
            });
        });

        $('.Card').ready( function () {
            parseFloat($('.Card-priceOld').innerHTML).toFixed(2);
        });
        ...
        $('.btn_download_file').click(function (event) {
            event.preventDefault();
            $('.fileTypePopupOverlay').fadeIn(297,function () {
                $('.fileTypePopup').css('display', 'block').animate({opacity:1}, 198);
            });
        });

        $('.fileTypePopupClose').click(function () {
            $('.fileTypePopup').animate({opacity: 1}, 198, function () {
                $(this).css('display', 'none');
                $('.fileTypePopupOverlay').fadeOut(297);
            });
                    ...
                    $('[data-refreshshow]').on('click change', function (e) {
                        ...
                        switch (data) {
                            ...
                            case'recent':
                                getData('/api/books/recent', {
                                    from: function() {
                                        var oldFrom = $('[data-refreshfrom]').data('refreshfrom');
                                        return oldFrom.replaceAll(".", "-");
                                    },
                                    to: function() {
                                        var oldTo = $('[data-refreshto]').data('refreshto');
                                        return oldTo.replaceAll(".", "-");
                                    },
                                    offset: function() {
                            ...
                            case'tag':
                                ...
                                break;
                            case'search':
                                getData('/api/books/search/' + $this.data('refreshquery'), {
                                    offset: function() {
                                        var oldOffSet = $this.data('refreshoffset');
                                        if(oldOffSet < 1) {
                                            $this.data('refreshoffset', ++oldOffSet);
            ...
            return {
                init: function () {
                    ...
                    $('[data-likeid]').on('click', function (e) {
                        ...
                        Login().postData('/api/rateBookReview', {
                            reviewId: $this.data('likeid'),
                            value: data.value
                        }, function (result) {
                            if (result != null && result.result) {
                                ...
                                $this.append('<div class="form-error">' + result.error + '</div>');
                                $("#err_dialog").dialog();
                            }
                        });
                    });
                ...
                init: function () {
                    ...
                    $('#submitPhone').on('click', function () {
                        ...
                        postData('/api/requestContactConfirmation', data, function (result) {
                            if (result.result) {
                                ...
                                $this.find('#submitPhoneCode').attr('disabled', 'enabled');
                            ...
                            console.log("Registration: "+result.code);
                        })
                    });
                    ...
                    $sendButton.on('click', function (e) {
                        ...
                        if ($this.is('[data-send]')) {
                            ...
                            $this.data('send', {contact: contact, code: 'login'});
                            if (!error) {
        ...
        var Profile = function () {
            ...
            return {
                init: function () {
                    ...
                    $('.profile_input_name').on('change', function () {
                        $('.btn_save_profile').show(0);
                    });
                // ,'input[name="phone"]''input[name="mail"]'
                    $('.form-input-profile').on('change', function () {
                        var $this = $(this),
                            $inputContact = $this;
                        $('.btn_save_profile').hide();
                        $this.next('.Profile-btn_confirm').show(0);
                        // $this.next('.form-input_code').show(0);
                        // $this.next('.form-input_code').enable(true);
                        $('.Profile-btn_confirm').on('click', function () {
                            var $this = $(this), $btnConfirm = $this, $inputCode = $this.next('.profile-input_code'),
                            ...
                            Login().postData('/api/requestContactConfirmation', data, function (result) {
                                if (result.result) {
                                    ...
                                    // $inputCode.attr('disabled', 'enabled');
                                } else {
                            ...
                            $inputCode.on('blur', function () {
                                ...
                                Login().postData('/api/approveContact', data, function (result) {
                                    if (result.result) {
                                        $this.after('<div class="form-result">Код успешно подтвержден</div>');
                                        $btnConfirm.hide();
                                        $btnConfirm.next('.form-info').hide();
                                        $this.hide();
                                        $('.btn_save_profile').show(0);
                                    } else {
                                        $this.addClass('form-input_error').after('<div class="form-error">' + result.error + '</div>');
                    ...
                    //     console.log(data);
                    //     Login().postData('/api/confirmProfile', data, function (result) {
                    //         if (result.result) {
                    //             $this.after('<div class="profile-save-confirm">Вам на E-Mail направлена ссылка для подтверждения изменений</div>');
                    //         } else {
                    //             $this.addClass('profile-save_error').after('<div class="form-error">' + result.error + '</div>');
                    //         }
                    //     });
                    //
                    // });
        ...
        var Tabs = function () {
            ...
            return {
                init: function () {
                    ...
                    // });
                    if ($payStatus.html() == "") {
                        $tabs.find($tabsLink).eq(0).trigger('click');
                    } else {
                        $tabs.find($tabsLink).eq(2).trigger('click');
                    }
        ...
        var Topup = function () {
            return {
                init: function () {
                    ...
                    $('.Cart-Buy').on('click', function (e) {
                        var $this = $(this);
                        e.preventDefault();
                        Login().postData('/payment', {
                            sum: $('.Cart-total').find('.Cart-price').text(),
                            time: new Date().getTime()
                        }, function (result) {

                            window.location.href = result.url+"?resultPay="+ result.message;
                            // $('.Cart-total').append('<div class="Topup-success">' + result.message + '</div>');
                        })
                    });
                }
            };
            ...
            function getData(address, data) {
                ...
                $.ajax({
                    ...
                    complete: function (result) {
                        if (result.status === 200) {
                            var data = '';
                            if (Object.keys(result.responseJSON.transactions).length < 5) {
